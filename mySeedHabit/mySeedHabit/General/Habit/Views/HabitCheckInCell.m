...
- (void)awakeFromNib {
    ...
    self.titleL.font = [UIFont fontWithName:@"Helvetica-Bold" size:17.f];
    [self.backgroundV addSubview:self.titleL];
    [self.titleL mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.mas_equalTo(self.backgroundV.mas_top).offset(10);
        make.left.mas_equalTo(self.backgroundV.mas_left).offset(40);
        make.size.mas_equalTo(CGSizeMake(SCREEN_WIDTH/2-60, 30));
    }];
    ...
    [self.iconV mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.left.mas_equalTo(self.check_in_timeL.mas_right).offset(5);
    ...
    for (int i = 0; i < self.count; i++) {
        ...
        [self.dataLabel mas_makeConstraints:^(MASConstraintMaker *make) {
            ...
            make.left.mas_equalTo(self.bgView.mas_left).offset(20+(float)i*((float)(SCREEN_WIDTH-60)/self.count));
        }];
